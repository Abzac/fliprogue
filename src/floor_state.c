...
void fr_save_current_floor(FrGame* game) {
    ...
    memcpy(floor->actors, game->actors, sizeof(game->actors));
    memcpy(floor->items, game->items, sizeof(game->items));
...
void fr_apply_saved_floor_state(FrGame* game, uint8_t floor_id) {
    ...
    for(uint8_t i = 0; i < floor->tile_delta_count; i++) {
        uint16_t pos = floor->tile_deltas[i].pos;
        ...
        game->tiles[y][x] =
            (uint8_t)((game->tiles[y][x] & ~(FR_TILE_TERRAIN_MASK | FR_TILE_HIDDEN_DOOR)) |
                      floor->tile_deltas[i].tile);
    }
    for(uint16_t pos = 0; pos < FR_MAP_W * FR_MAP_H; pos++) {
    ...
    memcpy(game->actors, floor->actors, sizeof(game->actors));
    memcpy(game->items, floor->items, sizeof(game->items));
